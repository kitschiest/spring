	...
	<c:if test="${!empty commentList}">
		<c:forEach var="rep" items="${commentList}">
			<div id="comment">
				...
				<div class="comment-confirm" style="display: none;">비밀번호 확인 : <input type="password" name="pwd_chk">
				</div>
				<p align="right">
	...
	<div class="comment-box">
		<form action="brInsert.do" id="replyForm" method="get">
			...
			<p align="center">
				...
				&nbsp;&nbsp;비밀번호 : 
				<input type="password" name="comment_pwd" size="23"><br> <br>
				<textarea id="reply_contents" class="form-control" rows="6"
					cols="70%" onfocus="if(this.value == 'Message') { this.value = ''; }" onblur="if(this.value == '') { this.value = 'Message'; }"
					placeholder="Message">
				</textarea>
				<br> <br> 
...
// 댓글 Insert Script 
	...
	if($('#reply_contents').val() == ""){ alert("내용을 입력해주세요.");
...
$(".updateConfirm").on('click',function(){ 
	...
	if(parentDiv.find('input[name=pwd_chk]').val() != parentDiv.children('input[name=rep_pwd]').val()){
		...
		return false; } else {
	$.ajax({
		url : "${pageContext.request.contextPath}/board/brUpdate.do", method : "POST",
		async : false,
		data: {
		...
		$(".delete").on('click',function(){
			...
			return false; } else {
			$.ajax({
			url : "${pageContext.request.contextPath}/board/brDelete.do", 
			method : "POST",
			data: {
			comment_id : parentDiv.find("input[name=rep_id]").val(),
			comment_pwd : parentDiv.find('input[name=pwd_chk]').val() },
